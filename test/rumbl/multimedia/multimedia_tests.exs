defmodule Rumbl.MultimediaTest do
  ...
  describe "videos" do
    ...
    test "list_videos/0 returns all videos" do
      ...
      assert [%Video{id: ^id1}] = Multimedia.list_videos() %Video{id: id2} = video_fixture(owner)
    ...
    test "create_video/2 with valid data creates a video" do
      owner = user_fixture()
      assert {:ok, %Video{} = video} = Multimedia.create_video(owner, @valid_attrs) assert video.description == "desc"
    ...
    test "update_video/2 with valid data updates the video" do
      ...
      assert {:ok, video} = Multimedia.update_video(video, %{title: "updated title"}) assert %Video{} = video
    ...
    test "update_video/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} = Multimedia.update_video(video, @invalid_attrs) assert %Video{id: ^id} = Multimedia.get_video!(id)
    end

    test "delete_video/1 deletes the video" do
      owner = user_fixture()
      video = video_fixture(owner)
      assert {:ok, %Video{}} = Multimedia.delete_video(video) assert Multimedia.list_videos() == []
    end
